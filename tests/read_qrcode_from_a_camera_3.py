...
def scrivi_in_ut_webcam(p_data):
    #Collegamento a Oracle
    try:
        v_oracle_db = cx_Oracle.connect(user='SMILE', password='SMILE', dsn='BACKUP_815')        
    except:
        return 'ko'
    ...
    v_oracle_cursor.execute("insert into ut_webcam values('"+getpass.getuser()+"','"+p_data+"')")                        
    # Committo
    v_oracle_db.commit()                                
    # Chiudo il cursore 
    ...
    while True:
        ...
        if v_webcam:
            ...
            if v_data is not None:                                
                # Disegna il mirino (verde se non ha letto nulla)                
                cv2.rectangle(img,(150-1,150-1),(350+1,300+1),(0,255,0),1)      
                cv2.line(img,(150,y-1),(350,y-1),(0,0,0),1)
                cv2.line(img,(150,y),(350,y),(0,255,0),1)        
                # Scrive il dato trovato nel DB di SMILE
                if scrivi_in_ut_webcam(str(v_data))=='ko':
                    cv2.putText(img,'Oracle error connection!', (10,470), font, 1,(0,0,255),1,cv2.LINE_AA)                          
                # e se il dato è stato scritto correttamente emetto messaggio e ....
                else:
                    cv2.putText(img,'Data found! ' + str(v_data), (10,470), font, 1,(255,255,255),1,cv2.LINE_AA)      
                # ... mi metto in attesa di un input dall'utente qualsiasi
                v_dato_da_utente = input()
